...
namespace Assignment_2_DIS
{
    class Program
    {
        ...
        Example 1:
        Input: nums = [1,3,4,5,6,7,8,9,10], target = 5
        Output: 2
        Example 2:
        ...
        */
        ...
        public static string MostCommonWord(string paragraph, string[] banned)
        {
            try
            {
                ...
                paragraph = Regex.Replace(paragraph, @"[^0-9a-zA-Z]+", " ");
                string[] paragraphTemp = paragraph.Split(" ");
                Dictionary<string, int> counts = new Dictionary<string, int>();
                foreach (string s in paragraphTemp)
                {
        ...
        */
        ...
        public static List<int> PartitionLabels(string s)
        {
            try
            {
                ...
                for (int i = 0; i < s.Length; i++)
                {
                    total++;
                    char c = s[i];
                    final = Math.Max(final, stringdict[c]);
                    if (i == final)
                    {
                        result.Add(total);
                        total = 0;
        ...
        public static List<int> NumberOfLines(int[] widths, string s)
        {
            try
            {
                    ...
                    foreach (Char i in c)
                    {
                        tempcount = widths[i - 97];
                        if (totalcount + tempcount > 100)
                        {
                        linecount++;
                ...
                return new List<int>  () { linecount, totalcount };
            }
            catch (Exception)
            {
                throw;
        ...
        */
        ...
        public static bool IsValid(string bulls_string10)
        {
            try
            {
                ...
                foreach (var item in bulls_string10.ToCharArray())    //  ([]) - ({[]})
                {
                    if (item == '(')
                        bracket.Push(')');
                    else if (item == '[')
                        ...
                        bracket.Push('}');
                    else if (bracket.Count == 0 || bracket.Pop() != item)
                        return false;
                }   
        ...
        */
        ...
        public static int UniqueMorseRepresentations(string[] words)
        {
            try
            {
                var MorseDict = new Dictionary<string, int>();
                ...
                foreach (string w in words)
                {
                    ...
                    foreach (char c in w)
                    {
                        temp = temp + Morse[(int)c % 32 - 1];
                    }
                    if (!MorseDict.ContainsKey(temp))
                    {
                        MorseDict.Add(temp, 1);
                    }
                    else
                    {
                        MorseDict[temp]++;
